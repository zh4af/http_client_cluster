...
import (
	...
	"third/golang.org/x/crypto/pkcs12"
)
...
func GetCertificate(filename, password string) (tls.Certificate, error) {
	switch path.Ext(filename) {
	case ".pem":
		bytes, err := ioutil.ReadFile(filename)
		if err != nil {
			log.Println("read pem file err: ", err)
	...
	case ".p12":
		...
		if err != nil {
			log.Println("read p12 file err: ", err)
			return tls.Certificate{}, err
		}
		return FromP12Bytes(p12bytes, password)
	default:
		log.Println("cert file invalid")
